#!/bin/bash
#          Script backup PostgreSQL  Jira-bacula       #
...
function mailer() {
	SUBJECTOK="${IDMAQUINA}-${TYPE}-OK";
	SUBJECTFAIL="${IDMAQUINA}-${TYPE}-ERROR";
...
for EXT in dmp dmp.gz log elog err sql; do
        for FILE in `find "${BACKUPDIR}" -daystart -maxdepth 2 -name "*.${EXT}" -mtime +${DIAS_A_MANTENER} -type f`;
        do
                rm -f $FILE
        done
done
